...
export default function App() {
  ...
  // Smooth Window Drag Handling across mouse movements
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (isDraggingLeft) {
        const newLeft = Math.max(200, Math.min(e.clientX, 550));
        setLeftWidth(newLeft);
      } else if (isDraggingRight) {
        const newRight = Math.max(250, Math.min(window.innerWidth - e.clientX, window.innerWidth - 300));
  ...
  return (
    <div className={`h-screen w-screen overflow-hidden bg-[#FFFFFF] text-[#242424] flex flex-row font-sans ${isDraggingLeft || isDraggingRight ? 'select-none cursor-col-resize' : ''}`}>
      ...
      {/* Drag Divider 1: Left - Center */}
      <div
        onMouseDown={() => setIsDraggingLeft(true)}
        className="w-1.5 hover:w-2 bg-[#5c7c89]/10 hover:bg-[#1f4959] cursor-col-resize active:bg-[#011425] transition-all flex-shrink-0 z-20 relative flex items-center justify-center group"
        title="Drag to resize Navigation Sidebar"
      >
        <div className="w-0.5 h-8 bg-[#5c7c89]/40 group-hover:bg-[#FFFFFF] rounded-full" />
      </div>
      ...
      {/* Drag Divider 2: Center - Graph View Window */}
      <div
        onMouseDown={() => setIsDraggingRight(true)}
        className="w-1.5 hover:w-2 bg-[#5c7c89]/10 hover:bg-[#1f4959] cursor-col-resize active:bg-[#011425] transition-all flex-shrink-0 z-20 relative flex items-center justify-center group"
        title="Drag to resize Graph View / Backlinks Window"
      >
        <div className="w-0.5 h-8 bg-[#5c7c89]/40 group-hover:bg-[#FFFFFF] rounded-full" />
      </div>
      ...
      <ClearCanvasModal
        ...
        onConfirmClear={handleConfirmClearCanvas}
